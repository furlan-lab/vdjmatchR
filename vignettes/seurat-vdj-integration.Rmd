...
#/Volumes/furlan_s/user/jperalta/MMCarT_240212_reseq/cds/mmcart_seu_viewmastR_CARpositive_251023.RDS
...
table(obj$nomatch)[2]/(table(obj$nomatch)[1]+table(obj$nomatch)[2])
...
if (exists("obj")) {
  ...
  if (nzchar(db_path) && file.exists(db_path)) {
    ...
    get_top_hits <- function(obj, chain = c("TRB", "TRA"), scope = "0,0,0,0") {
      ...
      keep_cols <- intersect(c("cell","alpha_rank","query_index","cdr3","v_segment","j_segment","gene","epitope","antigen.species","vdjdb_score","source"), colnames(hits))
      hits[, keep_cols, drop = FALSE]
    }

    top_trb <- get_top_hits(obj, chain = "TRB", scope = "2,1,2,3")
    ...
    if (!is.null(top_tra)) {
      cat("\nTop TRA hits (first 10, includes alpha_rank 1/2):\n")
      print(utils::head(top_tra, 10))
      if ("alpha_rank" %in% colnames(top_tra)) {
        cat("\nTRA hits count by alpha_rank:\n")
        print(table(top_tra$alpha_rank))
      }
    }
  } else {
