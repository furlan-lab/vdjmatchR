...
# macOS: propagate deployment target to Cargo builds to avoid ld warnings
if (Sys.info()[["sysname"]] == "Darwin") {
  dt <- tryCatch(system("R CMD config MACOSX_DEPLOYMENT_TARGET", intern = TRUE), error = function(e) "")
  # Collapse in case multiple lines are returned
  dt <- paste(dt, collapse = "\n")
  # If R doesn't know the variable or it is empty, fallback to env or 15.0
  if (length(dt) == 0 || dt == "" || grepl("no information for variable", dt, ignore.case = TRUE)) {
    dt <- Sys.getenv("MACOSX_DEPLOYMENT_TARGET", unset = "15.0")
  }
  new_txt <- gsub("@MACOSX_DEPLOYMENT_TARGET@", dt, new_txt)
} else {
  new_txt <- gsub("@MACOSX_DEPLOYMENT_TARGET@", "", new_txt)
}
